class KochanekBartels {
    static at(p0, p1, p2, p3, tension, continuity, bias, t, out) {
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // OPTIMIZATION NOTES :
        ...
        const d0a = ((1 - tension) * (1 + bias) * (1 + continuity)) * 0.5, d0b = ((1 - tension) * (1 - bias) * (1 - continuity)) * 0.5, d1a = ((1 - tension) * (1 + bias) * (1 - continuity)) * 0.5, d1b = ((1 - tension) * (1 - bias) * (1 + continuity)) * 0.5, d0x = d0a * (p1[0] - p0[0]) + d0b * (p2[0] - p1[0]), // Incoming Tangent
        d0y = d0a * (p1[1] - p0[1]) + d0b * (p2[1] - p1[1]), d0z = d0a * (p1[2] - p0[2]) + d0b * (p2[2] - p1[2]), d1x = d1a * (p2[0] - p1[0]) + d1b * (p3[0] - p2[0]), // Outgoing Tangent
        d1y = d1a * (p2[1] - p1[1]) + d1b * (p3[1] - p2[1]), d1z = d1a * (p2[2] - p1[2]) + d1b * (p3[2] - p2[2]);
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    static dxdy(p0, p1, p2, p3, tension, continuity, bias, t, out) {
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // OPTIMIZATION NOTES :
        // If interpolating a curve, TCB and Tangents shouldn't be calc for each point.
        // Precalc then reuse values for each t of the curve.
        // FOR splines, d0a, d0b, d1a, d1b Can be calced for all curves, then just do the tangents per curve.
        const d0a = ((1 - tension) * (1 + bias) * (1 + continuity)) * 0.5, d0b = ((1 - tension) * (1 - bias) * (1 - continuity)) * 0.5, d1a = ((1 - tension) * (1 + bias) * (1 - continuity)) * 0.5, d1b = ((1 - tension) * (1 - bias) * (1 + continuity)) * 0.5, d0x = d0a * (p1[0] - p0[0]) + d0b * (p2[0] - p1[0]), // Incoming Tangent
        d0y = d0a * (p1.y - p0.y) + d0b * (p2.y - p1.y), d0z = d0a * (p1.z - p0.z) + d0b * (p2.z - p1.z), d1x = d1a * (p2[0] - p1[0]) + d1b * (p3[0] - p2[0]), // Outgoing Tangent
        d1y = d1a * (p2[1] - p1[1]) + d1b * (p3[1] - p2[1]), d1z = d1a * (p2[2] - p1[2]) + d1b * (p3[2] - p2[2]);
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // Interpolate a point on the curve
        const tt = t * t;
