...
class Cycle {
    constructor(sec = 1) {
        // #region MAIN
        this._value = 0; // Current Cycle Value
        this._cycleInc = 0; // How much to move per millisecond
        this._speedScale = 1.0; // Scale the rate of the cycle
        this.setSeconds(sec);
    }
    // #endregion
    // #region METHODS
    setSeconds(s) { this._cycleInc = Maths.PI_2 / (s * 1000); return this; }
    /** Change Cycle to Update in a negative direction */
    backwards() { if (this._speedScale > 0)
        this._speedScale *= -1; return this; }
    /** Change Cycle to Update in a positive direction */
    forwards() { if (this._speedScale < 0)
        this._speedScale *= -1; return this; }
    ...
    get(offset = 0) { return Maths.mod(this._value + offset, Maths.PI_2); }
    /** Get Normalized Cycle Value - Range 0:1  */
    asCycle01(offset = 0) { return this.get(offset) * Maths.PI_2_INV; }
    /** Get Normalized Cycle Value - Range -1:1  */
    asCycleN11(offset = 0) { return this.get(offset) * Maths.PI_2_INV * 2.0 - 1.0; } // -1 : 1
    ...
    asSin(offset = 0) { return Math.sin(this.get(offset)); }
    /** Get Cycle with sin applied but range remaped to 0:1 */
    asSin01(offset = 0) { return Math.sin(this.get(offset)) * 0.5 + 0.5; }
    /** Get Cycle with sin applied but with absolute value applied. */
    asSinAbs(offset = 0) { return Math.abs(Math.sin(this.get(offset))); }
