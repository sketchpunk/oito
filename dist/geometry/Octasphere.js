...
class OctaSphere {
    ...
    static genVerts(subdiv, radius, vrows, verts, norm) {
    ...
    static repeatPoints(buf, offset, cnt, scl, verts, norm) {
    ...
    static genNormals(verts, out) {
        let v;
        for (v of Vec3.bufIter(verts))
            v.norm().pushTo(out);
    }
    // #endregion /////////////////////////////////////////////////////////////////////////////
    // #region INDICES
    // Create the Triangle indices for the Octasphere
    static genIndices(subdiv, vrows, out) {
    ...
    static arcLerp(a, b, seg_num, ary) {
        if (seg_num < 2)
            return; // Dont bother making points
