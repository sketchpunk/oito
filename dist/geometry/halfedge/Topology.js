import { EdgeCollection } from './Edge.js';
import { FaceCollection } from './Face.js';
import { VertexCollection } from './Vertex.js';
//#endregion
...
class Topology {
    ...
    getVertex(idx) { return this.vertices.items[idx]; }
    getHalfEdge(idx) { return this.edges.hedges[idx]; }
    getEdge(idx) { return this.edges.edges[idx]; }
    getFace(idx) { return this.faces.items[idx]; }
    //#endregion
    delVertex(vIdx) {
        const vert = this.getVertex(vIdx);
        const aryEdges = vert.getAllEdges(this);
        const aryFaces = new Set();
        let he, face, edg;
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // Get List of Faces
        for (edg of aryEdges) {
            ...
            if (he.face != -1)
                aryFaces.add(he.face);
            if ((face = he.getTwin(this).face) != -1)
                aryFaces.add(face);
        }
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
