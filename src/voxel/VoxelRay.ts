...
class VoxelRay{
    ...
    _init( ray: Ray, chunk: VoxelChunk, bbox: BoundingBox ): boolean{
        ...
        if( ray.dir[0] >= 0 ){  // Positive Direction
            ...
            if( ray.dir[ 0 ] > 0 ) this.xOut = chunk.dimension[ 0 ];
        }else this.xBound = this.ix * cellSize;

        if( ray.dir[1] >= 0 ){
            this.dir.y	= ( ray.dir[1] == 0 )? 0 : 1;
            this.yBound	= ( this.iy + 1 ) * cellSize;

            if( ray.dir[1] > 0 ) this.yOut = chunk.dimension[ 1 ];
        }else this.yBound = this.iy * cellSize;

        if( ray.dir[2] >= 0 ){
            this.dir.z	= ( ray.dir[2] == 0 )? 0 : 1;
            this.zBound	= ( this.iz + 1 ) * cellSize;

            if( ray.dir[2] > 0 ) this.zOut = chunk.dimension[ 2 ];
        }else this.zBound = this.iz * cellSize;
