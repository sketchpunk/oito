...
class FaceCollection{
    ...
    delVertex( fIdx: number, vIdx: number ): void{
        ...
        if( !lHe ){ console.log( 'Error that should never happen since Link Edge was just created' ); return; }
    ...
    add( verts: Array< number > | Array< TVec3 > ): number{
        ...
        if( isNaN( verts[ 0 ] ) ){
            ...
            let i;
            for( i of verts ) v.push( top.vertices.add( i ) );
            verts = v;
        }
        ...
        for( i=0; i < vLen; i++ ){
            ...
            //-------------------------------------------
            if( !heStart )            heStart       = he;       // Save First HalfEdge of Loop
            if( vert.halfedge == -1 ) vert.halfedge = he.index; // Assign HE to Vert
    ...
    /*
    addFace( verts: Array< number > | Array< TVec3 > ): number{
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if( verts.length == 0 ) return -1;
        if( isNaN( verts[ 0 ] ) ){
            const v : Array<number> = [];
            let i;
            for( i of verts ) v.push( this.top.vertices.add( i ) );
            verts = v;
        }
        ...
        const vLen = verts.length;
        const faceIdx = this.items.length;
        ...
        for( i=0; i < vLen; i++ ){
            ii  = ( i + 1 ) % vLen;
            edges.push( this.top.edges.add( verts[i], verts[ii], faceIdx ) );
        }
